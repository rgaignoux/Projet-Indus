...
skeleton_dilate = cv2.morphologyEx(skeleton, cv2.MORPH_DILATE, np.ones((5, 5)))
skeleton_copy = cv2.cvtColor(skeleton, cv2.COLOR_GRAY2BGR)

# Sobel filter to compute the gradient
sobel_x = cv2.Sobel(skeleton_dilate, cv2.CV_64F, 1, 0, ksize=7)
sobel_y = cv2.Sobel(skeleton_dilate, cv2.CV_64F, 0, 1, ksize=7)
