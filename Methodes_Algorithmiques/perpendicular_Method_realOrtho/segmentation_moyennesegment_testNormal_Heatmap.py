...
for road_path in  road_paths:
    ...
    for route in routes:
        ...
        mean_norm_x = np.max([norm_x for (norm_x, _) in normals.values()])
        mean_norm_y = np.max([norm_y for (_, norm_y) in normals.values()])
        widths1 = []
        widths2 = []
        ...
        for pos in points:
            ...
            for n in range(width1): 
                ...
                if x2 >= 0 and x2 < edges.shape[1] and y2 >= 0 and y2 < edges.shape[0]:
                    segmentation_mask[y2, x2] = width1
            ...
            for n in range(width2):
                ...
                if x3 >= 0 and x3 < edges.shape[1] and y3 >= 0 and y3 < edges.shape[0]:
                    segmentation_mask[y3, x3] = width2
    ...
    heatmap = cv2.applyColorMap(segmentation_mask_normalized, cv2.COLORMAP_JET)  # You can change the colormap if needed
    # Add a colorbar (legend)
    colorbar_height = 10  # Hauteur de la légende
    colorbar_width = heatmap.shape[1]  # Largeur de la légende égale à celle de la heatmap
    colorbar = np.zeros((colorbar_height, colorbar_width, 3), dtype=np.uint8)
    # Applique la heatmap sur la légende
    for i in range(colorbar_width):
        colorbar[:, i, :] = cv2.applyColorMap(np.array([[i * 255 // (colorbar_width - 1)]], dtype=np.uint8), cv2.COLORMAP_JET)[0, 0]

    combined_image = np.vstack((heatmap, colorbar))

    filename = os.path.splitext(os.path.basename(road_path))[0]
    cv2.imwrite(f"Methodes_Algorithmiques/perpendicular_Method_realOrtho//results_colormap//hitmap_{filename}.png", combined_image)
    if display == 1:
        utils.display_image("Segmentation",segmentation_mask)

    # Overlay the segmentation on the road image
    alpha = 0.6  # Poids de la heatmap (0 pour invisible, 1 pour totalement opaque)
    beta = 1 - alpha  # Poids de l'image d'origine (0 pour invisible, 1 pour totalement opaque)
    result = cv2.addWeighted(road, beta, heatmap, alpha, 0)
    cv2.imwrite(f"Methodes_Algorithmiques/perpendicular_Method_realOrtho//results_colormap//overlay_{filename}.png", result)
    if display == 1:
