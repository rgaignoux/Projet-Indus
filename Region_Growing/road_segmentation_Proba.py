...
##########################################################
# Obtenir la liste des fichiers axeX.png et routeX.png
axe_images = sorted(glob.glob('images/axe*.png'))  # images/axeX.png
road_images = sorted(glob.glob('images/route*.png'))  # images/routeX.png


# Boucle sur toutes les images
for idx_named, (axe_path, road_path) in enumerate(zip(axe_images, road_images)):
    print(f"Traitement de {axe_path} et {road_path}")
    ##########################################################
    # First Step : Mask the road image with the central axis #
    ##########################################################

    # Load the central axis image
    central_axis = cv2.imread(axe_path, cv2.IMREAD_GRAYSCALE)
    ...
    thresholds = [i for i in range (9,40)]  # Exemple de seuils différents : 9,40 c'est bien
    ...
    cv2.imwrite("resultats/Overlay"+str(idx_named)+".png", overlay_majority)
    # Affichage avec OpenCV
    #cv2.imshow("Overlay Majority - {axe_path}", overlay_majority)
